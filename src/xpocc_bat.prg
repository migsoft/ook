...
Procedure ook_xpocc_bat()
   ...
   FErase(HG_ROOT+'\Source\'+'ook_pocc_xhb.bat')
        ...
        Out := Out + cNewLi
        Out := Out + 'for %%a in (%HG_FILES1_PRG%) do if not errorlevel 1 %hg_pc%\%BIN_HRB%\pocc %OOHG_X_FLAGS% %%a.c' + cNewLi
        Out := Out + 'if errorlevel 1 goto EXIT2' + cNewLi
        Out := Out + 'for %%a in (%HG_FILES2_PRG%) do if not errorlevel 1 %hg_pc%\%BIN_HRB%\pocc %OOHG_X_FLAGS% %%a.c' + cNewLi
        Out := Out + 'if errorlevel 1 goto EXIT2' + cNewLi
        Out := Out + 'for %%a in (%HG_FILES_C%)    do if not errorlevel 1 %hg_pc%\%BIN_HRB%\pocc %OOHG_X_FLAGS% %%a.c' + cNewLi
        Out := Out + 'if errorlevel 1 goto EXIT2' + cNewLi
        Out := Out + 'if exist winprint.c  %hg_pc%\%BIN_HRB%\pocc %OOHG_X_FLAGS% winprint.c' + cNewLi
        Out := Out + 'if errorlevel 1 goto EXIT2' + cNewLi
        Out := Out + 'if exist miniprint.c %hg_pc%\%BIN_HRB%\pocc %OOHG_X_FLAGS% miniprint.c' + cNewLi
        Out := Out + 'if errorlevel 1 goto EXIT2' + cNewLi
        Out := Out + 'if exist bostaurus.c %hg_pc%\%BIN_HRB%\pocc %OOHG_X_FLAGS% bostaurus.c' + cNewLi
        Out := Out + 'if errorlevel 1 goto EXIT2' + cNewLi
        Out := Out + cNewLi
        ...
        Out := Out + '%hg_pc%\%BIN_HRB%\polib @%hg_root%\%LIB_GUI%\oohg.def' + cNewLi
        Out := Out + 'if errorlevel 2 goto EXIT3' + cNewLi
        Out := Out + 'if exist winprint.obj  %hg_pc%\%BIN_HRB%\polib /out:%hg_root%\%LIB_GUI%\hbprinter winprint.obj' + cNewLi
        Out := Out + 'if errorlevel 2 goto EXIT3' + cNewLi
        Out := Out + 'if exist miniprint.obj %hg_pc%\%BIN_HRB%\polib /out:%hg_root%\%LIB_GUI%\miniprint miniprint.obj' + cNewLi
        Out := Out + 'if errorlevel 2 goto EXIT3' + cNewLi
        Out := Out + 'if exist bostaurus.obj %hg_pc%\%BIN_HRB%\polib /out:%hg_root%\%LIB_GUI%\bostaurus bostaurus.obj' + cNewLi
        Out := Out + 'if errorlevel 2 goto EXIT3' + cNewLi
        Out := Out + cNewLi
        ...
        If !Empty(Main.Text_1.value) .and. !Empty(Main.Text_2.value) .and. !Empty(Main.Text_3.value)
            hb_Memowrit ( HG_ROOT+'\Source\'+'ook_pocc_xhb.bat' , Out )
            If File(HG_ROOT+'\Source\'+'ook_pocc_xhb.bat')
               MsgInfo("File: "+HG_ROOT+'\Source\'+'ook_pocc_xhb.bat'+" Created","Success...")
            Endif
        Endif
Return
